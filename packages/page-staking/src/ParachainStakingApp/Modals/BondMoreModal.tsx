...
function BondMoreModal ({ candidateAddress, className = '', delegatorAddress, onClose }: Props): React.ReactElement<Props> {
  ...
  return (
    <Modal
      ...
      header={t<string>('Stake More')}
      onClose={onClose}
      size='large'
    >
      <Modal.Content>
        <div className={className}>
          <Modal.Columns hint={t<string>('The staked amount will be subtracted (along with fees) from the delegator account.')}>
            <InputAddress
              defaultValue={delegatorAddress}
              help={t<string>('The account that has an active stake on the masternode candidate.')}
          ...
          <Modal.Columns hint={t<string>('Increase your stake by this amount.')}>
            {
              <InputBalance
                autoFocus
                help={<>{t<string>('The amount to increase the stake')}</>}
                isZeroable
